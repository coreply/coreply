import { and, rankWith, schemaMatches, uiTypeIs, type JsonSchema } from "@jsonforms/core";
...
function roundToStep(value: number, minimum: number, step: number, isInteger: boolean) {
  if (step <= 0) {
    ...
    <View className="mb-4 gap-2">
      {label && (
        <View className="flex-row items-center justify-between gap-3">
          ...
          <Text className="text-muted-foreground text-sm font-display">
            {displayValue}
          </Text>
        </View>
      )}
      ...
      <View className="flex-row justify-between">
        <Text className="text-muted-foreground text-xs font-display">{minimum}</Text>
        <Text className="text-muted-foreground text-xs font-display">{maximum}</Text>
      </View>
    </View>
  );
};
...
export const sliderTester = rankWith(
  ...
  and(
    uiTypeIs("Control"),
    schemaMatches((schema) => hasExplicitSliderBounds(schema as NumericJsonSchema)),
  ),
);
